...
module.exports = (robot) ->
  ...
  robot.respond /(task add|add task) (.+?)$/i, (msg) ->
    ...
    msg.send "お仕事追加しました: ##{task.num} - #{task.task}"

  robot.respond /(task list|list tasks|tasks)/i, (msg) ->
  ...
  robot.respond /(task delete|delete task|del task|task del) #?(\d+)/i, (msg) ->
    taskNum = msg.match[2]
    task = tasks.deleteByNumber taskNum
    msg.send "お仕事削除しました: ##{task.num} - #{task.task}"
